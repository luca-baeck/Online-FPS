...
public class GunShoot : MonoBehaviour
{
  ...
  private void Update()
  {
    if (!Input.GetButtonDown("Fire1") || (double) Time.time <= (double) this.nextFire || this.gunAim.GetIsOutOfBounds())
      return;
  ...
  private void SpawnDecal(RaycastHit hit, GameObject prefab) => Object.Instantiate<GameObject>(prefab, hit.point, Quaternion.LookRotation(hit.normal)).transform.SetParent(hit.collider.transform);
}
